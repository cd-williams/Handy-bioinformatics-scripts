...
# Same as BAMpath2IndSample.sh but returns a TSV with Indxxx and the corresponding BAM filepath. Because sometimes it is more convenient to filter by BAM filepath
...
while IFS= read -r line; do
    echo -e "${line}\tInd${i}" >> "${output_prefix}.tsv"
    ((i++))
